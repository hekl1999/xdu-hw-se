...
def change_course():
    ...
    if change_type == 'UPDATE':
        detail = post_data.get('detail')
        update_course = Course.query.filter_by(id=detail.get('course_id'))
...
def change_schedule():
    ...
    if change_type == 'UPDATE':
        update_schedule = Schedule.query.get(detail.get('schedule_id'))
        update_schedule.classes = Class.query.filter_by(
            id=detail.get('class_id', update_schedule.classes.id))
        update_schedule.classroom = Classroom.query.filter_by(
            id=detail.get('classroom_id', update_schedule.classroom.id))
